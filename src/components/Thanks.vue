<template>
  <section
    id="thanks"
    class="py-16 px-6 bg-gradient-to-b from-black to-stone-900 text-white">
    <div class="grid grid-cols-1 md:grid-cols-1 max-w-3xl mx-auto" data-aos="zoom-in">
      <div class=" text-center animate-fade-in">
        <p class="text-xl md:text-2xl leading-relaxed pb-6">
          Hadirnya Bapak, Ibu serta Teman-teman sekalian akan menjadi kebahagiaan dan kehormatan bagi kami, sebagaimana doa restu yang terucap menjadi cahaya bagi langkah kami.
        </p>

        <h2 class="text-3xl md:text-4xl mb-8 text-center animate-fade-in-delay font-serif">
          Nuraeni & Rivan
        </h2>
    
        <p class="font-light md:text-lg italic animate-fade-in-delay">
          Dengan penuh rasa syukur, kami mengucapkan terima kasih.
        </p>
      </div>
    </div>
  </section>
</template>
  